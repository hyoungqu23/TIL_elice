...
*함수는 함수가 생성되는 시점에 존재하는 위치에 있는 변수에 접근할 수 있다.*
```html
<script type="module">
...
여기서 각각 행은 하나의 개체, 관측치, 사례이고, 각각의 열은 속성, 변수, 특성, feature, field이다.
...
#### Click 시 페이지 전환 제거하기
`.preventDefault()`를 활용해 기본 동작을 막을 수 있다. 이때 this, event.target은 동일하고, event 객체 내 preventDefault가 존재한다.
```javascript
function nav() {
  objData.map((e) => {
    const $list = document.querySelector('nav ol');
...
### [Tic-Tac-Toe](https://codepen.io/hyoungqu23/pen/ZEvVpdo?editors=0010)
#### 주요 구문
```javascript
for (const box of boxes) { }
```
`event.currentTarget;` 현재 이벤트의 타겟
`image.src = owner === 'x' ? X_IMAGE_URL : O_IMAGE_URL` 삼항 연산자 할당
`const index = Math.floor(Math.random() * freeBoxes.length);`: 랜덤 인덱스 값 반환하기
...
`map()`: 각 요소에 대해 계산된 새로운 배열을 return으로 반환 받는다.
